...
import 'package:safe_streets/main_pages/map/path_search/service/path_service.dart';

import '../../../../shared/app_state.dart';
import '../../../../shared/global_functions.dart';
import '../models/response.dart';
import '../models/routes_response.dart';
...
class _PathSearch extends State<PathSearch> {
  ...
  Response response = Response(
    safestRouteResponse: RouteResponse(
      totalLength: 0.0,
      totalCrimeLikelihood: 0.0,
      edgesList: [],
    ),
    shortestRouteResponse: RouteResponse(
      totalLength: 0.0,
      totalCrimeLikelihood: 0.0,
      edgesList: [],
    ),
  );
  List<LatLng> safestPolylineCoordinates = [];
  List<LatLng> shortestPolylineCoordinates = [];
  ...
  Future<bool> _calculateDistance(
    ...
    try {
      ...
      // Mexico coordinates for a showcase
  ...
  setPolylines(double startLatitude, double startLongitude,
    ...
    // safest path (ML)
    response = (await pathService.getSafestAndFastestPath(startLatitude,
        startLongitude, destinationLatitude, destinationLongitude))!;

    // extract polylines of fastest route to show them on map
    for (final edge in response.shortestRouteResponse.edgesList) {
      List<List<double>> geometry = edge.geometry;
      for (final coordinates in geometry) {
        // lat = y, lng = x
        shortestPolylineCoordinates.add(LatLng(coordinates[1], coordinates[0]));
      }
      // create a single polylines
      Polyline polyline = Polyline(
        polylineId: PolylineId(edge.osmid),
        color: const Color.fromARGB(200, 0, 170, 136),
        points: shortestPolylineCoordinates,
    ...
    // show safest path on the map
    setState(() {
      // create polylines one-by-one with own id and color
      for (final edge in response.safestRouteResponse.edgesList) {
        List<List<double>> geometry = edge.geometry;
        for (final coordinates in geometry) {
          // lat = y, lng = x
          safestPolylineCoordinates.add(LatLng(coordinates[1], coordinates[0]));
        }
        // create a single polyline
        Polyline polyline = Polyline(
          polylineId: PolylineId(edge.osmid),
          color: _getColorByCrimeRate(edge.predCrimeLikelihood),
          points: safestPolylineCoordinates,
      ...
      // show on the map both paths
      _updatePathData();
    });

    // // show fastest path on the map
    // setState(() {
    ...
    //       color: const Color.fromARGB(200, 0, 170, 136),
    //       points: fastestPolylineCoordinates);
    //
  ...
  Widget build(BuildContext context) {
    return Align(
      ...
      child: Padding(
          padding: const EdgeInsets.all(8.0),
          child: isPathSearchShown
              ? showPathSearchContainer()
              : showPlaceholder()),
    );
  }
      ...
      return Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 10),
            Visibility(
              visible: response == null ? false : true,
              child: Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Text(
                    'DISTANCE: ${(response.safestRouteResponse.totalLength / 1000).toStringAsFixed(2)} km',
                  ...
                  Text(
                    ', SAFETY: ${response.safestRouteResponse.totalCrimeLikelihood.toStringAsFixed(2)}',
                    style: const TextStyle(
                      fontSize: 16,
                      fontWeight: FontWeight.bold,
                    ),
                  ...
                  onPressed: (_startAddress != '' && _destinationAddress != '')
                      ? () async {
                          ...
                          setState(() {
                            ...
                            if (safestPolylineCoordinates.isNotEmpty) {
                              safestPolylineCoordinates.clear();
                            }
                            if (shortestPolylineCoordinates.isNotEmpty) {
                              shortestPolylineCoordinates.clear();
                            }
                            // add to the main-map via callback function
                            _updatePathData();
                          });
